...
//				}
	...
	private synchronized boolean isChating(int room_id) {
		//消息房间号是否与当前聊天一样
		return chatActivity != null && !chatActivity.isFinishing()
...
//				map.put("mtime", mtime + "");
	...
	public void setMsgAdapter(BaseListFragment chatFragment, int room_id) {
		if(msgAdapter == null) {
			msgAdapter = chatFragment.getListAdapter();
		}
			this.chatActivity = chatFragment.getActivity();
			ChatSocketClient.chatFragment = chatFragment;
			currentRoomId = room_id;
	}
			...
			if (msg.what == DOWN_LOAD_ATTACH) {
								...
								new Thread(new Runnable() {
									...
									public void run() {
										...
										if (message.getMessage_id() != 0) {
											long result = SQLHelperChatMessage.addChatMessagetoChatList(
													message,
				...
				try {
					...
					if (message_wh.getMessage_id() != 0) {
						long result = SQLHelperChatMessage.addChatMessagetoChatList(message_wh,message_wh.getMessage_id());
					}
