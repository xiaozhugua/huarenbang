package com.abcs.sociax.t4.android.checkin;
...
import com.abcs.sociax.api.ApiCheckin;
...
public class ActivityCheckIn extends ThinksnsAbscractActivity {
	...
	private void initData() {
		...
		Date date = new Date(c.get(Calendar.YEAR) - 1900,
				Integer.valueOf(mMonth) - 1, Integer.valueOf(mDay));
		SimpleDateFormat dateFm = new SimpleDateFormat("EEEE");
		tv_shadow.setText(mMonth+"月"+mDay+"日  "+dateFm.format(date));
		tv_time.setText(mMonth+"月"+mDay+"日  "+dateFm.format(date));


		Object info = Thinksns.getCheckInfo();
		if(info != null) {
			Log.i("xuke19","签到之后的数据:"+info.toString());
			setCheckData(info);
		}
	}
	...
	private void setCheckData(Object o) {
		try {
			...
			int month = Integer.parseInt(mad.substring(0, 2));
			int day = Integer.parseInt(mad.substring(3, 5));
