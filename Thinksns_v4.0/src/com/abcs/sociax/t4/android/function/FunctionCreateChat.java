			...
			case StaticInApp.CREATE_GROUP_CHAT:
				try {
					...
					if (result.getString("status").equals("1")) {
						...
						intent.putExtra("issingle", msg.arg1 == 0);
...
//			List<ModelChatMessage> list = chatHelper.getChatList();
	...
	public void createChat(final boolean isSingle) {
		if (this.userList.size() == 0)
			return;
		final int from_uid = Thinksns.getMy().getUid();
		...
		if (isSingle==true){
			...
			userface = userList.get(0).getUserface();
			Log.d(TAG, "((ModelSearchUser) userList.get(0))="+ userList.get(0).toString());
