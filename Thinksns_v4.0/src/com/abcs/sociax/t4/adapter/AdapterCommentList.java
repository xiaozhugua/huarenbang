...
import com.abcs.sociax.t4.android.data.AppendComment;
...
public class AdapterCommentList extends AdapterSociaxList {
	...
	public ListData<SociaxItem> refreshFooter(SociaxItem obj)
		...
		try {
			if(type.equals("post")) {
				return getApi().getPostCommentList(10, feed_id, getMaxid(), httpListener);
			}else {
				return new Api.WeiboApi().getWeioComments(feed_id, getMaxid(), 10, httpListener);
			}
	...
	public ListData<SociaxItem> refreshNew(int count)
			...
			try {
				if(type.equals("post")) {
					return getApi().getPostCommentList(10, feed_id, 0, httpListener);
				}else {
					return new Api.WeiboApi().getWeioComments(feed_id, 0, 10, httpListener);
				}
			} catch (ExceptionIllegalParameter e) {
				e.printStackTrace();
			}
