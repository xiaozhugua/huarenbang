...
public class FormPost {
	...
	public static String postMultilPic(String actionUrl,HashMap<String, String> params, FormFile[] formFiles) {
		String result = null;
		try {
			// 通知栏
			NotificationManager notiManager = (NotificationManager) Thinksns.getContext().getSystemService(Activity.NOTIFICATION_SERVICE);
			Notification notification = new Notification();
			notification.flags = Notification.FLAG_AUTO_CANCEL;
			notification.icon = R.drawable.app_load;
			...
			for (FormFile file : formFiles) {
				...
				while ((len = is.read(buffer)) != -1) {
					...
					if (percent >= perFlag) {
						notification.contentView.setTextViewText(
								R.id.content_view_text1, (Math.round(percent*100)/100) + "%");
						notification.contentView.setProgressBar(
								R.id.content_view_progress, totalSize,
								1024 * i, false);
						notiManager.notify(0, notification);
			...
			Log.v("ActivityCreateWeibo", "postMultilPic: result"+result);
	...
	public static String postPicOnly(String actionUrl,
		...
		Log.v("result", result);
		Log.v("ActivityCreateWeibo", "postPicOnly: result"+result);
		return result;
	}
}
