...
public class MarkView extends ListView {
	...
	public void getData() {
		...
		String url = TLUrl.getInstance().URL_wxh + (isAll ? "/getall" : "/get");
		String param = isAll ? "" : ("uid=" + MyApplication.getInstance().self
				.getId());
				...
				try {
					if (isAll) {
						...
						while (iterator.hasNext()) {
							...
							for (int i = 0; i < jsonArray.length(); i++) {
								...
								mark.setInfo(object.optString("gnjs"));
								mark.setName(object.optString("name"));
								mark.setAvatar(object.optString("purl"));
								mark.setType(object.optInt("type"));
								mark.setUrl(object.optString("url"));
								mark.setNumber(object.optString("weixinhao"));
								mark.setMark(activity.isMark(mark.getId()));
					...
					handler.post(new Runnable() {
						...
						public void run() {
							...
							if (!isAll
									&& activity.viewpager.getCurrentItem() == 0)
								activity.noMark.setVisibility(list.size() == 0 ? View.VISIBLE
										: View.GONE);
						}
					});
	...
	private BaseAdapter adapter = new BaseAdapter() {
		...
		public View getView(int position, View v, ViewGroup parent) {
			...
			if (v == null) {
				...
				v.setTag(holder);
				holder.grp = (CustomListView) v
						.findViewById(R.id.tljr_view_item_newsmark_grp);
				holder.name = (TextView) v
						.findViewById(R.id.tljr_view_item_newsmark_name);
				holder.arrow = (ImageView) v
						.findViewById(R.id.tljr_view_item_newsmark_arrow);
				...
				holder.newsMark.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						ViewHolder holder = ((ViewHolder) ((View) v.getParent())
								.getTag());
						holder.arrow
								.setRotation(holder.arrow.getRotation() == 90 ? -90
										: 90);
						holder.grp
								.setVisibility(holder.arrow.getRotation() == 90 ? View.VISIBLE
										: View.GONE);
					}
				});
			...
			holder.name.setText(String2AlphaFirst(list.get(position).get(0)
					.getName(), "b"));
			((MyBaseAdapter) holder.grp.getAdapter()).setList(list
					.get(position));
