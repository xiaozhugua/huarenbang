 ...
 */
package com.abcs.huaqiaobang.tljr.news.animations;
...
/**
 * A BaseAdapterDecorator class which applies multiple Animators at once to views when they are first shown. The Animators applied include the animations specified in
...
public abstract class AnimationAdapter extends BaseAdapterDecorator {
	...
	/**
	 * Call this method to reset animation status on all views. The next time notifyDataSetChanged() is called on the base adapter, all views will animate again.
	 */
	public void reset() {
		mAnimators.clear();
		mLastAnimatedPosition = -1;
	...
	public void resetAtPostion(int index) {
		mAnimators.clear();
		mLastAnimatedPosition = index;
		mAnimationStartMillis = -1;
	...
	/**
	 * Set whether this AnimationAdapter is encapsulated by another AnimationAdapter. When this is set to true, this AnimationAdapter does not apply any animations to the views.
	 * Should not be set explicitly, the AnimationAdapter class manages this by itself.
	 */
	...
	/**
	 * Get the Animators to apply to the views. In addition to the returned Animators, an alpha transition will be applied to the view.
	 * 
	 * @param parent
