...
//        Log.d("ChartActivity", "answers :"+ask.answers);
       JSONArray arr = JSONArray.parseArray(ask.answers);
       if(arr.size()>0){
           holder.rl.setVisibility(View.VISIBLE);
           JSONObject ob = arr.getJSONObject(0);
           AskBean a = new AskBean();
           a.id = ob.getString("id");
           a.isfocus = ob.getBooleanValue("isfocus");
...
//           holder.time2.setText(Util.getDateOnlyHour(ob.getLongValue("date")));
           holder.time2.setText(Util.getDateOnlyDat(System.currentTimeMillis()).equals(Util.getDateOnlyDat(ob.getLongValue("date")*1000))
        		   ?Util.getDateOnlyHour(ob.getLongValue("date")*1000) : Util.getDateOnlyDat(ob.getLongValue("date")*1000));
